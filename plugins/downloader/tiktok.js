        ...
        try {
            ...
            if (parseUrl) {
                let res = await Func.fetchJson(
                    `https://www.tikwm.com/api/?url=${parseUrl}&hd=1`
                );
                if (!res || !res.data)
...
Mengirim.....`);
                ...
                if (data.images && data.images.length > 0) {
                    if (data.images.length < 2) {
                        ...
                        await conn.sendAlbumMessage(m.chat, media, {
                            quoted: m
                        });
                    }
                } else {
                ...
                if (data.music_info.play) {
                    ...
                    m.reply(
                        "Musik tidak ditemukan, hanya akan mengirimkan media itu saja."
                    );
                }
            } else if (input) {
                let search = await Func.fetchJson(
                    `https://www.tikwm.com/api/feed/search?keywords=${input}&count=1&cursor=0&web=1&hd=1`
                );
                let video = search?.data?.videos[0];
                if (!video)
                    return m.reply(
                        `Video tidak ditemukan untuk pencarian "${input}".`
                    );
...
`.trim();
                ...
                m.reply({
                    video: { url: "https://www.tikwm.com" + video.play },
                    caption
                });
            } else {
