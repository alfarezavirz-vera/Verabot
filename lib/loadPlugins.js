...
async function importModule(modulePath) {
    ...
    try {
        ...
        let plugins = esm?.default ?? esm;
        // Deteksi kalau plugin adalah class (punya prototype.run)
        if (typeof plugin === "function" && plugin.prototype?.run) {
            plugin = new plugin();
        }
        return plugins;
    } catch (error) {
...
export default class PluginsLoad {
    constructor(directory, { debug = false } = {}) {
        if (!directory) {
            throw new Error(
                "You must add plugins path before using this code!"
            );
        }
    ...
    async load() {
        await this.scan();
        log.info(
            `Successfully loaded ${Object.keys(this.plugins).length} plugins`
        );
