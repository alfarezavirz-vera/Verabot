   ...
   const client = Object.defineProperties(conn, {
      ...
      sendSticker: {
         async value(jid, filePath, m, options = {}) {
            ...
            const sticker = await (await import('./exif.js')).writeExif({
               mimetype: mime,
               data
            }, exif);
...
export default async function serialize(conn, msg) {
         ...
         if (typeof text === 'string') {
            return conn.sendMessage(m.chat, {
               ...
               contextInfo: {
                  ...
                  externalAdReply: {
                     title: global.cfg.ads.title,
                     body: global.cfg.ads.body,
                     showAdAttribution: false,
                     mediaType: 1,
                     previewType: 0,
                     renderLargerThumbnail: false,
                     thumbnailUrl: global.cfg.ads.imageUrl,
                     sourceUrl: global.cfg.ads.sourceUrl,
                  },
                  isForwarded: true,
                  forwardingScore: 2,
   ...
   m.react = (jid, emote) => conn.sendMessage(jid, {
      react: {
         text: emote,
         key: m.key
      }
   ...
   m.delet = (query) => conn.sendMessage(m.chat, {
      delete: query
   })
   m.edit = (query, key) => conn.sendMessage(m.chat, {
      text: query,
      edit: key
   })
